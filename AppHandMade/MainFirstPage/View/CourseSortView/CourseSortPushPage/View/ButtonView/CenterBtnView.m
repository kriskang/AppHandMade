...
@interface CenterBtnView ()<UICollectionViewDataSource, UICollectionViewDelegate>
@property (nonatomic, retain) UICollectionView *collectionView;
@property (nonatomic, retain) UICollectionViewFlowLayout *flowLayout;
@property (nonatomic, retain) NSArray *timeArray;
@property (nonatomic, retain) NSArray *pub_timeArray;

@end
@implementation CenterBtnView
- (void)dealloc
{
    [_collectionView release];
    [_flowLayout release];
    [_timeArray release];
    [_pub_timeArray release];
    Block_release(_centerBtnBlock);
    [super dealloc];
}
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.timeArray = @[@"一周", @"一月", @"全部教程"];
        self.pub_timeArray = @[@"week", @"month", @"all"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.titleLabel.text = self.timeArray[indexPath.item];
    return cell;
}
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    self.centerBtnBlock(self.timeArray[indexPath.item], self.pub_timeArray[indexPath.item]);
}
- (void)layoutSubviews
{
    [super layoutSubviews];
    self.collectionView.frame = self.bounds;
    [self.flowLayout setItemSize:CGSizeMake(350*Screen_W, 40*Screen_H)];
